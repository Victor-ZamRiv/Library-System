...
function obtenerRangoFechas() {
    ...
    if (periodoTexto === 'SEMANA' || periodoTexto === 'Semana') desdeDate.setDate(hoy.getDate() - 7);
    else if (periodoTexto === 'MES' || periodoTexto === 'Mes') desdeDate.setMonth(hoy.getMonth() - 1);
    else if (periodoTexto === 'TRIMESTRE' || periodoTexto === 'Trimestre') desdeDate.setMonth(hoy.getMonth() - 3);
    return {
...
async function descargarPDF(tipo) {
    ...
    try {
        ...
        doc.text(`Fecha de emisión: ${new Date().toLocaleString()}   |   Período: desde ${rango.desde} hasta ${rango.hasta}`, 14, 51);
        ...
        if (config.customResumen) {
            ...
            if (cumPanel) {
                const raw = cumPanel.querySelector('.label')?.innerText || '';
                const match = raw.match(/\d+(\.\d+)?%/);
                resumenRows.push(['Cumplimiento de Plazos de Préstamo', match ? match[0] : '--', limpiarTextoEstado(cumPanel.querySelector('.label'))]);
            }
            if (document.getElementById('chartOcupacion')) {
                const raw = document.getElementById('badgeOcupacionEstado')?.innerText || '';
                const match = raw.match(/\d+(\.\d+)?%/);
                resumenRows.push(['Ocupación Diaria de Salas', match ? match[0] : '--', limpiarTextoEstado(document.getElementById('badgeOcupacionEstado'))]);
            }
            if (document.getElementById('panel-participacion-actividades')) {
                const pBar = document.getElementById('panel-participacion-actividades').querySelector('.progress-bar');
                const v = pBar ? pBar.innerText.trim().split(' ')[0] : '--';
            ...
            doc.setFontSize(10);
            doc.setFont("helvetica", "bold");
            doc.text("RESUMEN DE LA OPERACIÓN Y METADATOS", 14, startY);
            
            doc.setFontSize(9);
            doc.setFont("helvetica", "normal");
            
            const lineasInfo = infoText.split('|').map(l => l.trim());
            let infoY = startY + 6;
            lineasInfo.forEach(linea => {
                doc.text(linea, 14, infoY);
                infoY += 5;
            });
            ...
            const itemsAntes = document.querySelectorAll('.panel-warning .list-group-item');
            const itemsAhora = document.querySelectorAll('.panel-success .list-group-item');

            itemsAntes.forEach((itemAntes, index) => {
                const itemAhora = itemsAhora[index];
                if (!itemAntes || !itemAhora) return;

                const etiqueta = itemAntes.querySelector('strong')?.innerText.replace(':', '').trim() || 'Desconocido';
                
                const clonAntes = itemAntes.cloneNode(true);
                const clonAhora = itemAhora.cloneNode(true);
                
                clonAntes.querySelector('strong')?.remove();
                clonAhora.querySelector('strong')?.remove();
                
                const valorAntes = clonAntes.innerText.trim();
                const valorAhora = clonAhora.innerText.trim();
                
                const esCambio = itemAntes.classList.contains('cambio') || itemAhora.classList.contains('cambio');

                dRows.push([etiqueta, valorAntes, valorAhora, esCambio]);
            });

            doc.autoTable({
                head: config.headers,
                body: dRows.map(r => [r[0], r[1], r[2]]),
                startY: startY,
                theme: 'grid',
                headStyles: { fillColor: [31, 78, 121], halign: 'center', fontSize: 9, fontStyle: 'bold' },
                styles: { fontSize: 8.5, cellPadding: 3.5 },
                columnStyles: {
                    0: { cellWidth: 50, fontStyle: 'bold', fillColor: [249, 249, 249] }
                },
                margin: { left: 14, right: 14 },
                didParseCell: function(data) {
                    if (data.cell.section === 'body') {
                        const filaOriginal = dRows[data.row.index];
                        const esCambio = filaOriginal ? filaOriginal[3] : false;
        ...
        } else if (config.multiTable) {
            const container = document.querySelector(config.targetId);
            if (container) {
                const elements = container.querySelectorAll('.report-title, .table-report');
                for (let i = 0; i < elements.length; i++) {
                    const el = elements[i];
                    if (el.classList.contains('report-title')) {
                        if (startY + 20 > pageUsableHeight) {
                            doc.addPage();
                            startY = 20;
                        }
                        doc.setFontSize(11);
                        doc.setFont("helvetica", "bold");
                        doc.setTextColor(44, 62, 80);
                        doc.text(el.innerText.trim().toUpperCase(), 14, startY);
                        startY += 7;
                    } else if (el.classList.contains('table-report')) {
                        doc.autoTable({
                            html: el,
                            startY: startY,
                            theme: 'grid',
                            ...
                            didParseCell: function(data) {
                                if (data.cell.element && (data.cell.element.classList.contains('bg-gray') || data.cell.element.parentElement.classList.contains('bg-gray'))) {
                                    data.cell.styles.fillColor = [240, 240, 240];
                                    data.cell.styles.textColor = [0, 0, 0];
                                    data.cell.styles.fontStyle = 'bold';
                                }
                                if (data.cell.element && data.cell.element.classList.contains('text-left')) {
